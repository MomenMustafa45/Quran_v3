// GRDBManagerImpl.swift
import Foundation
import GRDB

@objc(GRDBManagerImpl)
class GRDBManagerImpl: NSObject {
  ...
  private func openDatabaseInternal() throws -> DatabaseQueue {
    if let dbQueue = dbQueue { return dbQueue }
    guard let dbPath = getDatabasePath() else {
      throw NSError(domain: "GRDBManager", code: 1,
  ...
  @objc func openDatabase(_ resolve: @escaping RCTPromiseResolveBlock,
  ...
  @objc func closeDatabase(_ resolve: @escaping RCTPromiseResolveBlock,
                            rejecter reject: @escaping RCTPromiseRejectBlock) {
    dbQueue = nil
    resolve(true)
  }

  @objc func isDatabaseOpen(_ resolve: @escaping RCTPromiseResolveBlock,
                             rejecter reject: @escaping RCTPromiseRejectBlock) {
    resolve(dbQueue != nil)
  }
      ...
      try dbQueue.read { db in
        ...
        if let params = parameters {
          ...
          for (_, value) in sortedParams {
            switch value {
            case let v as String: arguments.append(v)
            case let v as Int: arguments.append(v)
            case let v as Double: arguments.append(v)
            case let v as Bool: arguments.append(v)
            default: arguments.append(Optional<String>.none as DatabaseValueConvertible)
            }
          }
        }
        let rows = try Row.fetchAll(db, sql: query, arguments: StatementArguments(arguments))
        let resultRows = rows.map { $0.toDictionary() }
        resolve(["rows": resultRows, "rowsAffected": resultRows.count, "insertId": -1])
      }
    } catch {
      reject("DATABASE_ERROR", error.localizedDescription, error)
    }
  ...
  @objc func executeRawQuery(_ query: String,
                              resolver resolve: @escaping RCTPromiseResolveBlock,
                              rejecter reject: @escaping RCTPromiseRejectBlock) {
    do {
      let dbQueue = try openDatabaseInternal()
      try dbQueue.write { db in
        ...
        resolve(["rowsAffected": db.changesCount, "insertId": db.lastInsertedRowID])
      }
    } catch {
      reject("DATABASE_ERROR", error.localizedDescription, error)
    }
...
extension Row {
  func toDictionary() -> [String: Any] {
    ...
    for column in columnNames {
      ...
      case let v as Data: dict[column] = v.base64EncodedString()
      case .none: dict[column] = NSNull()
      default: dict[column] = "\(value)"
      }
    }
    return dict
  }
