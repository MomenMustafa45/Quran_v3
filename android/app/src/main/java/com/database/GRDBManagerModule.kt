...
class GRDBManagerModule(private val reactContext: ReactApplicationContext) :
    ...
    override fun getName() = "GRDBManager"
    ...
    private fun copyDatabaseIfNeeded() {
        ...
        val input = reactContext.assets.open(DATABASE_NAME)
                ...
                for (i in 0 until map.size) {
                    ...
                    if (it.hasKey(key)) {
                        ...
                        when (it.getType(key)) {
                            ...
                            ReadableType.String ->
                                params.add(it.getString(key))

                            ReadableType.Number ->
                                params.add(it.getDouble(key).toString())

                            ReadableType.Boolean ->
                                params.add(if (it.getBoolean(key)) "1" else "0")
                ...
                for (i in 0 until cursor.columnCount) {
                    ...
                    when (cursor.getType(i)) {
                        ...
                        Cursor.FIELD_TYPE_STRING ->
                            row.putString(column, cursor.getString(i))

                        Cursor.FIELD_TYPE_INTEGER ->
                            row.putInt(column, cursor.getInt(i))

                        Cursor.FIELD_TYPE_FLOAT ->
                            row.putDouble(column, cursor.getDouble(i))

                        Cursor.FIELD_TYPE_BLOB ->
                            row.putString(
                                column,
                                Base64.encodeToString(cursor.getBlob(i), Base64.DEFAULT)
                            )
